...
function App() {
  ...
  const fetchICSFile = () => {
    fetch('http://localhost:5000/api/download')
      .then((res) => res.text())
      .then((icsData) => {
        console.log("Fetched ICS content:", icsData);
    ...
    <>
        ...
        <div className="logo-design">
          <div>
            <img 
              ...
              alt="Group Scheduler" 
              style={{float: "left"}}
              onClick={() => setShowGroupSchedule(true)}
            />
          </div>
          <div>
      ...
      <div className="main-container">
		...
		<input type="text" placeholder="Ask Buzz..." value={message} onChange = {(e) => setMessage(e.target.value)} onKeyDown={handleKeyDown}/>
          ...
          {mode === "add" && chatResponse && chatResponse.length > 0 ? (
            chatResponse.map((opt) => (
              <div key={opt.rank} className="event-option">
                {/*Display opt.rank to show readable version instead*/}
                <pre>{opt.rank}</pre>
                <button onClick={() => handleSelectEvent(opt.event)}>Select</button>
              </div>
            ))
          ) : (
